...
{
	/// <summary>
	/// Summary description for VerticalBoxProfile.
	/// </summary>
	public unsafe class VerticalBoxProfile : BoxProfile
	{
		private object _syncObject = new object();
		private Thread _workerThread = null;
        private float _selectedY = float.NaN;
        private PointF _pt1 = PointF.Empty, _pt2 = PointF.Empty;

		public VerticalBoxProfile(DataProfileHelper container)
			: base(container)
		{
		}
		...
		public override void Render(Graphics graph, Rectangle rcClip)
		{
			...
			if (float.IsNaN(_selectedY) == false)
			{
				PointF pt1 = new PointF(this.Rectangle.Left, _selectedY);
				PointF pt2 = new PointF(this.Rectangle.Right, _selectedY);
		...
		public override void UpdateSelectedValue(object abscissaValue, object ordinaryValue)
		{
            try
            {
                _selectedY = (float)abscissaValue;
		...
		private void WorkerThread()
		{
			try
			{
                ...
                this.DlgLineProfile.UpdateBoxPlot(this.Settings as BoxProfileSettings, rect, data, categories);
			}
			catch
			{
			}
