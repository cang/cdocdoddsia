...
{
    public class DetectContaminationsCommandBrightPoleTip : RasterCommand
    {
        private ArrayList _detectedObjects = null;

        public DetectContaminationsCommandBrightPoleTip(IProgressCallback callback)
            : base(callback)
        {
        }
        ...
        protected override void OnRun(params object[] args)
        {
            ...
            try
            {
                IMask mask = ContaminationTexturalInfoHelperPoleTip.Mask;

//                 this.SetStatusText("Extracting Features...");
//                 FeatureSpace featureSpace =
//                     FeatureExtractor.ExtractFeatures_For_PoleTipContaminationsDetection(image);
//                 if (featureSpace == null)
//                     return;
// 
//                 string nnsfile = Path.Combine(
//                     SettingsHelper.GetDefaultPoleTipInspectionSettingsFolderPath(),
//                     "PoleTip_Contamination_NNModel.nns");
// 
...
// 
                ...
                double threshold = settings.HigherIntensityThreshold_PoleTip;

                binaryImage = Binarize(image, mask, threshold);

                _detectedObjects = ObjectDetection.DetectObject(image, binaryImage);
                /*HARD CODE => IMPLEMENT TO SETTTINGS IN FUTURE*/
                for (int i = _detectedObjects.Count - 1; i >= 0; i--)
                {
                    DetectedObject obj = (DetectedObject)_detectedObjects[i];
                    if (obj.NumPixels < 40)
                    {
                        _detectedObjects.RemoveAt(i);
                    }
                }
