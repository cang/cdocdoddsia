...
namespace SIA.Workbench 
{
    ...
    internal class MainForm : System.Windows.Forms.Form 
	{
		...
		private ucScriptBuilder _scriptBuilder = null;
		private ucScriptExecutor _scriptExecutor = null;
		...
		private void InitializeComponent() 
		{
			...
			this.toolbarRDEW.Size = new System.Drawing.Size(780, 44);
			...
			// statusbarMain
			// 
			this.statusbarMain.Location = new System.Drawing.Point(0, 540);
			...
			this.statusbarMain.Size = new System.Drawing.Size(780, 22);
			...
			this.statusBarPanelCurrentSubStep.Width = 764;
			// 
			// timerStatusUpdate
			// 
		...
		public void DoCommandOpenScript() 
		{
			...
			using (OpenFileDialog dlg = CommonDialogs.OpenScriptFileDialog("Open Script File")) 
			{
		...
		public bool DoCommandSaveScript(bool bSaveAs) 
		{
			...
			if (bSaveAs) 
			{
				using (SaveFileDialog dlg = CommonDialogs.SaveScriptFileDialog("Save script...")) 
				{
...
//					#endregion old version to check logic, will improve later
		...
		{
			this._scriptBuilder = new SIA.Workbench.UserControls.ucScriptBuilder();
			this._scriptBuilder.Dock = DockStyle.Fill;
			panelScriptBuilder.Controls.Add(this._scriptBuilder);
			_scriptExecutor = new ucScriptExecutor();
		...
		private void UpdateGUIStatus(bool bLock) 
		{
			if (_isExecutionMode) 
			{
				...
				else 
				{
					...
					mnStopScript.Enabled = false;					
					mnSaveLogFile.Enabled = this._scriptExecutor.Logs != string.Empty;
					mnClearLogs.Enabled = this._scriptExecutor.Logs != string.Empty;
					...
					cmdSaveLogFile.Enabled = this._scriptExecutor.Logs != string.Empty;
					cmdClearLogs.Enabled = this._scriptExecutor.Logs != string.Empty;
					#endregion update toolbar status
				}
			}
			else 
		...
		private void UpdateWindowTitle() 
		{
			if (this._workingSpace != null && this._workingSpace.FileName!=null && this._workingSpace.FileName!=string.Empty)
				this.Text = MainForm.Title + " - " + this._workingSpace.FileName;
			else
				this.Text = MainForm.Title + " - " + WorkingSpace.defaultTitle;
		}
		...
		protected virtual void OnEndProcessFile(EventArgs args) 
		{
			...
			this.statusBarPanelCurrentSubStep.Text = string.Empty;
		}
		...
		protected virtual void OnStatusChanged(EventArgs args) 
		{
			...
			this.statusBarPanelCurrentSubStep.Text = this._scriptExecutor.CurrentSubStep;
		}
