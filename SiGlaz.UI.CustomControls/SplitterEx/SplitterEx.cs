...
namespace SiGlaz.UI.CustomControls
{
	#region Enums
	/// <summary>
	/// Enumeration to sepcify the visual style to be applied to the SplitterEx control
	/// </summary>
	...
	/// </summary>
	[ToolboxBitmap(typeof(SplitterEx))]
	//[DesignerAttribute(typeof(SplitterExDesigner))]
	public class SplitterEx : System.Windows.Forms.Splitter
	{
		...
		/**
		public bool DrawBorderLeft = false;
		public bool DrawBorderRight = false;
		public bool DrawBorderTop = false;
		public bool DrawBorderBottom = false;
		private Color _borderColor = Color.FromArgb(0, 45, 150);
		**/


		/**
		private GradientColor _bkColor = new GradientColor(
		...
		**/
		...
		protected override void OnPaint(PaintEventArgs e)
		{
			...
			if (this.Dock == DockStyle.Left || this.Dock == DockStyle.Right)
			{
				...
				rr = new Rectangle(r.X, (int)r.Y + ((r.Height - 115) / 2), 5, 115);
				...
				if (this.Enabled)
				{
					...
					g.FillPolygon(new SolidBrush(Color.Blue), ArrowPointArray(rr.X + 1, rr.Y + 3));
					g.FillPolygon(new SolidBrush(Color.Blue), ArrowPointArray(rr.X + 1, rr.Y + rr.Height - 9));
				}
			...
			else if (this.Dock == DockStyle.Top || this.Dock == DockStyle.Bottom)
			{
				...
				rr = new Rectangle((int)r.X + ((r.Width - 115) / 2), r.Y, 115, 5);
				...
				if (this.Enabled)
				{
					// draw the arrows for our control image
					// the ArrowPointArray is a point array that defines an arrow shaped polygon
					g.FillPolygon(new SolidBrush(Color.Blue), ArrowPointArray(rr.X + 3, rr.Y + 1));
					g.FillPolygon(new SolidBrush(Color.Blue), ArrowPointArray(rr.X + rr.Width - 9, rr.Y + 1));
				}
			...
			#region DrawBorder
			/**
			if (DrawBorderLeft || DrawBorderRight || DrawBorderTop || DrawBorderBottom)
			{
