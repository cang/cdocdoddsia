...
{
    internal class CmdAnalysisAlignABS : AppWorkspaceCommand
    {
        public const string cmdCommandKey = "CmdAnalysisAlignABS";
		
		private static MenuInfo menuInfo = null;
		private static ToolBarInfo toolbarInfo = null;
		private static ShortcutInfo shortcutInfo = null;
        
		static CmdAnalysisAlignABS()
		{
            // 
            Image image = SIAResources.GetMenuIcon("align_ABS");
            menuInfo = 
                new MenuInfo("&Align ABS", Categories.Analysis, Shortcut.None, 600, image, SeparateStyle.None);
            toolbarInfo = 
                new ToolBarInfo("Align ABS", "Align ABS", 600, image, SeparateStyle.Before);
            shortcutInfo =
                new ShortcutInfo("Align ABS", Categories.Analysis, 600, image);			
		}

        public CmdAnalysisAlignABS(IAppWorkspace appWorkspace)
		...
		public override void DoCommand(params object[] args)
        {
            ...
            {
                // begin process
                callback = workspace.BeginProcess("Align ABS...", false, ProgressType.Normal);
                ...
                if (alignSettings.UseDefault)
                    settings = Settings.GetDefaultABSAlignmentSettings();
                else
                ...
                {
                    object[] result = cmdExec.DoCommandAlignABS(image, settings);
                    if (result == null)
        ...
        private AlignABSCommandSettings LoadDefault()
        {
            try
            {
                string fileName = Path.Combine(
                    Application.UserAppDataPath, @"SiGlaz\SIA\DefaultSettings\AlignABS.settings");
