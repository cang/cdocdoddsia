...
namespace SIA.UI
{
	...
	public class NiceMenu : MenuItem 
	{
		...
		private static NiceMenu[] _myModifyNiceMenu = new NiceMenu[20];
		private static NiceMenu[] _myModifyContextNiceMenu = new NiceMenu[20];
		private static int _contModify = 0;
		...
		private void BuildMenuTree(IList myMenu, MainMenu parentMenu, NiceMenuClickEvent yourClickFunction)
		{
			...
			foreach (MenuItem myMenuItem in myMenu) 
			{
				...
				if (_myModifyNiceMenu[_contModify] == null)
				{
					_myModifyNiceMenu[_contModify] = newMainMenu;
					_contModify ++;
				}
		...
		private void BuildMenuTree(IList myMenu, ContextMenu parentMenu, NiceMenuClickEvent yourClickFunction)
		{
			...
			foreach (MenuItem myMenuItem in myMenu) 
			{
				...
				if (_myModifyContextNiceMenu[contModifyContext] == null)
				{
					_myModifyContextNiceMenu[contModifyContext] = newMainMenu;
					contModifyContext ++;
				}
		...
		public NiceMenu SearchNiceMenuItem(string NiceMenuText)
		{
			try
			{
				foreach (NiceMenu myMenuitem in _myModifyNiceMenu)
					if (myMenuitem != null && myMenuitem.Text == NiceMenuText)
						return myMenuitem;

				_contModify = 0;
			
			repeat:
				IList myMenuList = new Menu.MenuItemCollection(_myModifyNiceMenu[_contModify]);
				...
				foreach (NiceMenu myMenuItem in myMenuList) 
				{
					...
					if (myMenuItem.IsParent == true)
					{
						...
						if(myMenuItem.Text !="Recent Files")
						{
			...
			{
				Trace.WriteLine("Cannot find menu item " + NiceMenuText);
			}
			finally
			{
			}
		...
		public NiceMenu SearchContextNiceMenuItem(string ContextNiceMenuText)
		{
			...
			repeat:
				// ---------------------------------------
				if (_myModifyContextNiceMenu[contModifyContext].Text 
					== ContextNiceMenuText)
				{
					return _myModifyContextNiceMenu[contModifyContext];
				}
			// --------------------------------------------
			IList myMenuList = new Menu.MenuItemCollection(_myModifyContextNiceMenu[contModifyContext]);
			foreach (NiceMenu myMenuItem in myMenuList) 
			{
